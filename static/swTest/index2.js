$(document).ready(function () {
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    ...
    refreshing = true;
    console.log('5：刷新页面' + new Date().getTime())
  ...
  // 刷新
  function emitUpdate() {
    console.log('2：有新版本，显示刷新按钮 ' + new Date().getTime());
    $('.sw-update').show();
  ...
  // 手动刷新
  $(".sw-update").click(function () {
    try {
      console.log('3：手动刷新：sw-update ' + new Date().getTime())
  ...
  if ('serviceWorker' in navigator) {
    ...
    navigator.serviceWorker.register('./sw2.js').then(function (reg) {
      console.log("1：Register success: ", reg.scope);
          ...
          switch (installingWorker.state) {
            case 'installed':
              ...
              if (navigator.serviceWorker.controller) {
                console.log('1.1：已更新版本 ' + new Date().getTime())
                emitUpdate();
              }
              break;
          }
      ...
      console.error('2：Error during service worker registration:', e);
    });
  } else {
