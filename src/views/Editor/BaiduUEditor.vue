<template>
  <div class="content">
    ...
    <vue-ueditor-wrap ref="ueditor1" v-model="msg" :destroy="false" :config="config"
                      @ready="ready" @beforeInit="addCustomUI"></vue-ueditor-wrap>
    <br>
    <vue-ueditor-wrap ref="ueditor2" v-model="msg" :destroy="false" :config="config"
                      @ready="ready" @beforeInit="addCustomUI"></vue-ueditor-wrap>
    <br>
      ...
      addCustomUI(editorId, editorConfig) {
        ...
        window.UE.registerUI('test-button' + editorId, function (editor, uiName) {
          ...
          var btn = new window.UE.ui.Button({
            ...
            cssRules: "background-image: url('/static/UEditorCustom/test-button.png') !important;background-size: cover;",
            // 点击时执行的命令
            onclick: function () {
        ...
        window.UE.registerUI('table-center-button' + editorId, function (editor, uiName) {
          ...
          var btn = new window.UE.ui.Button({
            ...
            title: '点击设置表格居中',
            // 需要添加的额外样式，可指定 icon 图标，图标路径参考常见问题 2
            cssRules: "background-image: url('/static/UEditorCustom/center.png') !important;background-size: cover;",
            // 点击时执行的命令
            onclick: function () {
              // 这里可以不用执行命令，做你自己的操作也可
              editor.execCommand(uiName)
        ...
        window.UE.registerUI('test-dialog' + editorId, function (editor, uiName) {
          ...
          var dialog = new window.UE.ui.Dialog({
            ...
            iframeUrl: '/static/UEditorCustom/customizeDialogPage.html',
            // 需要指定当前的编辑器实例
            editor: editor,
            // 指定 dialog 的名字
            name: uiName,
          ...
          var btn = new window.UE.ui.Button({
            name: 'dialog-button',
            title: '鼠标悬停时的提示文字',
            cssRules: `background-image: url('/static/UEditorCustom/test-dialog.png') !important;background-size: cover;`,
            onclick: function () {
              // 渲染dialog
              dialog.render()
              dialog.open()
